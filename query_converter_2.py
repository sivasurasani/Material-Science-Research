    ...
    for query in queries:
        ...
        for entry in filtered_data:
            ...
            for key, value in query.items():
                ...
                sub_entry = entry
                for k in keys:
                    if isinstance(sub_entry, dict) and k in sub_entry:
                ...
                # Check the value match based on type
                if isinstance(value, tuple):
                    if not (value[0] <= sub_entry <= value[1]):
                        match = False
                        break
                elif isinstance(sub_entry, (int, float, str)) and sub_entry != value:
                    match = False
                    break
                elif isinstance(sub_entry, list) and key == 'lemma':
